...
public class InMemoryMatchHistoryRepositoryTest {
    private final InMemoryMatchHistoryRepository matchHistoryRepository = new InMemoryMatchHistoryRepository();
    ...
    void loadUpdatedAfterIncrease() {
        // Arrange
        matchHistoryRepository.trackOutcome(GameMode.MISTER_X, WON);
    ...
    void increaseWorksProperly(int runnerWins, int runnerLoses, int seekerWins, int seekerLoses) {
        ...
        for (int i = 0; i < runnerLoses; i++) {
            matchHistoryRepository.trackOutcome(GameMode.MISTER_X, LOST);
        }

        for (int i = 0; i < seekerWins; i++) {
            matchHistoryRepository.trackOutcome(GameMode.DETECTIVE, WON);
        }

        for (int i = 0; i < seekerLoses; i++) {
            matchHistoryRepository.trackOutcome(GameMode.DETECTIVE, LOST);
        }
