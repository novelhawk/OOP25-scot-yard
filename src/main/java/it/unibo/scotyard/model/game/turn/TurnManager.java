...
/**
 * This interface defines the contract for managing player turns, including move validation and execution.
 *
...
public interface TurnManager<M extends TurnManager.MoveOption> {
    ...
    /**
     * Represents a valid move option in the game. A move consists of a destination node and the transport type used to
     * reach it.
     */
    ...
    /**
     * Calculates all valid moves from the current position. A move is valid if there is a connection to the destination
     * node via the specified transport.
     *
     * @param currentPosition the current node position
     * @param occupiedPositions the set of node IDs currently occupied by other players
